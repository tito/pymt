'''
Dejitter: A problem that is often faced (esp. in optical MT setups) is that of
...
class InputPostprocDejitter(object):
    ...
    jitter_distance is a float in range 0-1.
    jitter_ignore_devices is a comma-seperated list of device identifiers that
    should not be processed by dejitter (because they're very precise already).
    """
    def __init__(self):
        self.jitterdist = pymt.pymt_config.getfloat('pymt', 'jitter_distance')
        ignore_devices = pymt.pymt_config.get('pymt', 'jitter_ignore_devices')
