...
repeat_delta = MTButton(label="Repeat Delta",style={'bg-color':(0,0,0)},height=50)
but_layout.add_widget(repeat_delta)


repeat_abs = MTButton(label="Repeat Abs",style={'bg-color':(0,0,0)},height=50)
but_layout.add_widget(repeat_abs)

but_layout.x = m.width/2-but_layout.width/2 #Align button at the bottom-center


#Add Objects
greeny = MTSprite(filename="icons/greeny.png" ,pos=(100,100), style={'scale':0.5})
m.add_widget(greeny)
...
mov_dxy = Animation(duration=0.5, pos=(50,50), animation_type="delta")
mov_dx = Animation(duration=0.5, x=50, animation_type="delta")
rot_d = Animation(duration=0.5, rotation=45, animation_type="delta")
...
repeat_abs_anim = Repeat(mov_x & rot, times=6)

test = Repeat(Animation(duration=1, style={'scale':0.5}, animation_type="delta"),times=5)
